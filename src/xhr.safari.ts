(function() {
   const SAFARI_INSTAGRAM_XHR_MSG = 'safari-ig-dl-xhr-msg';

   const safariOldXHROpen = window.XMLHttpRequest.prototype.open;
   ...
   window.XMLHttpRequest.prototype.open = function (method, url) {
      if (method === 'GET' && typeof url === 'string') {
         if (url.includes('/api/v1/feed/reels_media/?reel_ids=')) {
            this.addEventListener('load', function () {
               window.postMessage({
                  type: SAFARI_INSTAGRAM_XHR_MSG,
                  payload: { data: this.responseText, api: '/api/v1/feed/reels_media/?reel_ids=' }
               }, '*');
            });
         }
      }
         ...
         switch (url) {
            ...
            case 'https://www.instagram.com/graphql/query':
               this.addEventListener('load', function () {
                  window.postMessage({
                     type: SAFARI_INSTAGRAM_XHR_MSG,
                     payload: { api: 'https://www.instagram.com/graphql/query', data: this.responseText }
                  }, '*');
            ...
            case '/api/graphql':
               this.addEventListener('load', function () {
                  window.postMessage({
                     type: SAFARI_INSTAGRAM_XHR_MSG,
                     payload: { api: 'https://www.instagram.com/api/graphql', data: this.responseText }
                  }, '*');
               });
               break;
            default:
      ...
      return safariOldXHROpen.apply(this, [].slice.call(arguments) as any);
   };
})();
